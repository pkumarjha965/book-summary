...
def test_login_returns_token_for_valid_credentials(mock_get_user):
    response = client.post("/token", data={"username": "valid_user", "password": "valid_password"})
    assert response.status_code == 200
    assert "access_tokenn" in response.json()



@patch("service.datahandler.get_user_from_db", return_value=None)
def test_login_returns_401_for_invalid_user(mock_get_user):
...
def test_login_returns_401_for_invalid_password(mock_verify, mock_get_user):
    response = client.post("/token", data={"username": "valid_user", "password": "invalid_password"})
    assert response.status_code == 401


@patch("service.datahandler.get_user_from_db", side_effect=get_mock_user)
def test_get_current_user_returns_user_for_valid_token(mock_get_user):
...
@patch("service.datahandler.get_user_from_db", return_value=None)
def test_get_current_user_returns_401_for_invalid_token(mock_get_user):
...
def test_get_books_returns_list_of_books(mock_get_books):
...
def test_get_books_returns_500_on_exception(mock_get_books):
    response = client.get("/book")
...
def test_get_book_by_id_returns_book_details(mock_get_book):
...
def test_get_book_by_id_returns_404_if_book_not_found(mock_get_book):
...
def test_create_book_returns_successful_response(mock_create_book):
...
def test_create_book_returns_500_on_exception(mock_create_book):
    response = client.post("/book",
                           json={"title": "New Book", "author": "Author", "genre": "Fiction", "year_published": 2023,
                                 "summary": "Summary"})
    assert response.status_code == 500
